      ...
      <script type="text/javascript">
          ...
          $(document).ready(function() {
              ...
              var calendar = $('#calendar').fullCalendar({
                  ...
                  firstDay: 1, // Lundi : premier jour de la semaine
                  dayMaxEvents: true, // allow "more" link when too many events
                  columnHeaderHtml: function(args) {    // Mise en forme des dates en en-tête
                    let string = args.format('ddd');
                    window.mobilecheck() ? format = string[0].toUpperCase() + string.slice(1)+'<br>'+ args.format('D/M') : format = string[0].toUpperCase() + string.slice(1)+' '+ args.format('D/M');
                    return format;
                  },
                  events: "{{ route('book.index') }}",
                  displayEventTime: true,
